...
class UserTest(APITestCase):
    ...
    def setUp(self):
        ...
        self.anonymous_user_2 = User.objects.create(
            username="gera2", email="gera2@gatepass.com.ar", is_verified=False
        )
    ...
    def test_list_users(self):
        ...
        response = self.client.get(url, format="json")
        self.assertEqual(response.data["results"][0]["username"], "gera_logged")
        self.assertEqual(
            response.data["results"][0]["email"], "gera_logged@gatepass.com.ar"
        )
        self.assertEqual(response.status_code, status.HTTP_200_OK)
