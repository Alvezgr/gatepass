...
class VehiclesKindTests(APITestCase):
    ...
    def test_fail_create_vehicle_camion(self):
        ...
        response = self.client.post(url, data, format="json")
        self.assertEqual(response.status_code, status.HTTP_400_BAD_REQUEST)
        self.assertEqual(response.data["name"][0], "Este nombre de vehiculo ya existe.")
...
class VehicleTests(APITestCase):
    ...
    def test_update_vehicle(self):
        ...
        data = {"insurance_expiration": "2026-03-02"}

        self.client.force_authenticate(user=self.user)
        response = self.client.patch(url, data, format="json")
        self.assertEqual(response.status_code, status.HTTP_200_OK)
        self.assertEqual(
            response.data["insurance_expiration"], data["insurance_expiration"]
        )
